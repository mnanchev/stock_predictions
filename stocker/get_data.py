...
def main(stock, years=1):  # function to get data from Yahoo Finance
    ...
    start = (dt.datetime.today() - dt.timedelta(days=365 * years)).strftime(
        "%Y-%m-%d"
    )  # 1 year ago as start
...
def company_name(stock):  # function to get the company's name from the stock
    ...
    try:
        company = requests.get(url).json()["ResultSet"]["Result"][0][
            "name"
        ]  # saving the name as 'company'
...
def get_interest(
    company, timeframe
):  #  base function to get 'interest' from Google Trends
    (pytrend := TrendReq()).build_payload(
        kw_list=[company], timeframe=timeframe
    )  # accessing to Google Trends using pytrends package and finding interest for 'company' during 'timeframe'
    return (
        result := pytrend.interest_over_time().drop("isPartial", axis=1)
    )  # saving the 'interest' values


def add_interest(
    df, company, years=1
):  # main function to get 'interest' from Google Trends
    ...
    for x in range(delta):  # get the values for the rest of the year
        since = (dt.datetime.today() - dt.timedelta(days=73 * (delta - x))).strftime(
            "%Y-%m-%d"
        )
        until = (
            dt.datetime.today() - dt.timedelta(days=73 * (delta - 1 - x))
        ).strftime("%Y-%m-%d")
        timeframe = since + " " + until
        trends.append(get_interest(company, timeframe))

    trends.rename(
        columns={company: "Interest"}, inplace=True
    )  # changing title to 'Interest'
    trends.index.names = ["Date"]
    return (df := df.merge(trends, how="left", on="Date")).Interest.interpolate(
        inplace=True
    )  # Add Interest column from Google Trends API - pytrends and interpolation for missing values


def add_wiki_views(
    df, company, start, end
):  # function to get number of page views from Wikipedia
    start = start.replace("-", "")
    end = end.replace("-", "")
    link = (
    ...
    r = requests.Session()
    r.headers = {
        "User-Agent": "stocker/0.1.7 (https://github.com/jcamiloangarita/stocker; camiloang94@gmail.com)"
    }
    response = r.get(link)
    ...
    date = [
        dt.datetime.strptime(date[:-2], "%Y%m%d").date().strftime("%Y-%m-%d")
        for date in date
    ]  # change format
    ...
    return (df := df.merge(wiki_views, how="left", on="Date")).Wiki_views.ffill(
        inplace=True
    )  # Add Wiki_views column from Wikipedia API
...
def add_rsi(df, period):  # function to Calculate RSI values
    ...
    df["Again"] = df.Gain.rolling(
        period
    ).mean()  # calculate the average gain in the last 14 periods
    df["Aloss"] = df.Loss.rolling(
        period
    ).mean()  # calculate the average loss in the last 14 periods

    df["RS"] = df.Again / df.Aloss  # calculating RS
    df["RSI"] = 100 - (100 / (1 + (df.Again / df.Aloss)))  # calculating RSI
    df.drop(
        columns=["Gain", "Loss", "Again", "Aloss", "RS"], inplace=True
...
def add_k(df, period):  # Calculate Stochastic Oscillator (%K)
    df["L14"] = df.Low.rolling(
        period
    ).min()  # find the lowest price in the last 14 periods
    df["H14"] = df.High.rolling(
        period
    ).max()  # find the highest price in the last 14 periods
...
def add_r(df, period):  # Calculate Larry William indicator (%R)
    df["HH"] = df.High.rolling(
        period
    ).max()  # find the highest high price in the last 14 periods
    df["LL"] = df.Low.rolling(
        period
    ).min()  # find the lowest low price in the last 14 periods
...
def total(
    stock, years=1, interest=False, wiki_views=False, indicators=False, period=14
):
    # main function to combine data from Yahoo Finance, Google Trends, Wikipedia and calculated indicators.
    df, start, end = main(
        stock, years=years
    )  # get data from Yahoo Finance and define star and end
    company = None
    ...
    if company is not None:
        if interest:
            df = add_interest(
                df, company, years=years
            )  # adding Interest from Google Trends.
        if wiki_views:
